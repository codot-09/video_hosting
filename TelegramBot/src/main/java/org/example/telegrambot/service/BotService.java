...
public class BotService extends TelegramLongPollingBot {
        ...
        if (text != null && text.equalsIgnoreCase("/start")) {
            ...
            if (allSubscribed) {
                ...
                for (String channelLink : notSubscribedChannels) {
                    ...
                    button.setText("Obuna bo'lish: " + channelLink);
            ...
            if (text.equalsIgnoreCase("Statistika")) {
                ...
                if (commands.contains(text)) {
                    ...
                    response = new SendMessage(chatId.toString(), "Avvalgi protsess to'xtatildi. Yangi buyruq ishlatilmoqda: " + text);
                    response = handleTextMessage(update, chatId, text, response);
                } else {
                ...
                sendReminder(chatId,"Kanal qo'shish uchun shu kanalga botni admin qiling aks holda majburiy obunada ushbu kanal ishlatilmaydi");
    ...
    private void handleCallbackQuery(Update update, Long chatId, String callbackData) {
        if (callbackData.equals("check_subscription")) {
            ...
            int bookIndex = Integer.parseInt(callbackData.substring(5));
            List<Book> books = userBookSearches.get(chatId);

            if (books != null && bookIndex < books.size()) {
                Book selectedBook = books.get(bookIndex);
                sendBookDetails(chatId, selectedBook);
            } else {
                sendErrorMessage(chatId, "❌ Xatolik: Tanlangan kitob topilmadi.");
            }
